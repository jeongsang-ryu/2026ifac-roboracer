...
1. 팀들은 대화 전 등록 절차에서 자신의 차에 대한 기술 자료를 제출해야 합니다.
...
6. _메인 컴퓨팅 유닛_:
    사양에 **제한 없음**. 한개의 컴퓨닝 유닛만 사용 가능합니다.
7. _라이다_:
...
10. _배터리_:
    **4s LiPo 배터리** 또는 **3S 이하**. 하나의 4S 배터리 또는 낮은 셀 조합만 허용 (예: 2s + 2s 허용).
11. _감지 박스_:
...
- 팀은 토글 방식으로 비상 브레이크를 작동시킬 수 있음을 시연해야 합니다(단, **경주 중 개입용으로는 사용할 수 없습니다**!).

- 차량 검사는 첫째 날 대회에서 실시됩니다.

- 검사는 경주 심판이 수행합니다.

- 검사는 **타임 트라이얼 전**에, 그리고 차량의 하드웨어나 알고리즘에 **중대한 변경 후**에 완료되어야 합니다.
...
- 이 시간은 맵핑 뿐만 아니라 데이터 취득 및 연습 또한 가능합니다.
...
- 이번 대회에서는 트랙에 대한 png,yaml 파일을 제공합니다.
...
- 차량이 피트스톱 존에 있으면, 페널티 없이 차량 하드웨어 수리 및 파라미터 튜닝이 가능합니다.
- 이 구역에서는 일반적인 트랙과 마찬가지로 사람이 서있는 것은 금지됩니다.
- 추가적으로, 이 구역으로 안전하게 진입 후에 수리를 위해 차량을 섬으로 빼는 것은 경고에 해당하지 않는 상황입니다.
- 자율 레이싱 상황에서 이 구역을 완전히 침범하는 것은 페널티 대상입니다.

### 2.5.1 진입
- 사람이 하는 주행은 **수동 주행 구역**에서 피트스톱 존에 진입하기 위해서만 사용할 수 있습니다.
...
- 예선은 총 **3가지의 미션**과 **1가지의 달성 조건**으로 이루어져 있습니다. 이는 순서대로 Q1, Q2, Q3, Q4로 지칭합니다.
...
- 최종순위 결정의 예시는 다음과 같습니다.
    - 총 12팀이 있다고 가정.
    - 각 Q1, Q2, Q3, Q4 통과팀이 각각 3팀이라고 가정.
    - Q4를 통과한 팀 중에서 Q3의 기록 순으로 1위부터 3위까지 배정.
...
### 2.6.8 주의 사항
- 예선 도중 차량을 **임의의 위치(예: 시작선)**로 가져가는 것은 **엄격히 금지**됩니다.
